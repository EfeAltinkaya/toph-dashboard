...
const PLOTS = [
  { field: "Field A", reading: "Spraying · 24 oz/acre", col: 11, row: 2, w: 4, h: 3, show: "hidden lg:block", delay: "0s", crosshair: true, labelRight: false },
  { field: "Field C", reading: "Scouting · no product", col: 13, row: 6, w: 2, h: 2, show: "hidden lg:block", delay: "1.1s", crosshair: false, labelRight: false },
  // Nearest the right edge, so its label hangs off the box's right side
  // instead of running past the viewport.
  { field: "Field B", reading: "Harvest · 6:42 AM", col: 16, row: 3, w: 3, h: 2, show: "hidden xl:block", delay: "2.2s", crosshair: false, labelRight: true },
];
...
export function FieldScan() {
  ...
  return (
    <section className="relative isolate min-h-[36rem] overflow-hidden bg-soil">
      <Image
        src={PHOTOS.fieldRowsAerial.src}
        alt={PHOTOS.fieldRowsAerial.alt}
        fill
        sizes="100vw"
      ...
      <div className="mx-auto flex min-h-[36rem] max-w-6xl items-center px-6 py-20">
        <div className="max-w-md">
          ...
          <h2 className="mt-3 font-display text-4xl leading-tight text-wheat sm:text-5xl">
            The ground, turned into data you can audit.
          </h2>
          <p className="mt-4 text-wheat/70">
            Every log is pinned to real coordinates and checked against what
            was applied, so the record says exactly which block was treated,
            with what, and when.
          </p>
          <dl className="mt-6 space-y-1.5 font-eyebrow text-[11px] tracking-widest text-wheat/50 uppercase">
            <div className="flex gap-3">
              <dt className="text-scan">Field A</dt>
              <dd>
                {formatCoord(fieldA.lat, "N", "S")} · {formatCoord(fieldA.lng, "E", "W")}
              </dd>
            </div>
            <div className="flex gap-3">
              <dt className="text-scan">Mapped</dt>
              <dd>{Object.keys(FIELD_COORDS).length} fields · Firebaugh, CA</dd>
            </div>
          </dl>
          <Link
            href="/product"
